...
#estimate day of 25% of growth----


#loop through list of each site id to get day by which 25% of cumulative growth has occurred
#str(gpp_df)
#id_list <- as.factor(unique(gpp_df$id))
id_list <- unique(gpp_df$id_value)
#doy_25_list <- list()

#get typical day by which 25% of growth has occurred
with_progress({
...
gpp_25_df <- do.call('rbind', gpp_25_list)
gpp_25_df <- rasterFromXYZ(gpp_25_df)
crs(gpp_25_df) <- "+proj=longlat +datum=WGS84"
#plot(gpp_25_df)
filename <-
  paste0('./../../Data/CDD/day_of_25/day_25_', Ecoregion, '.tif')
writeRaster(gpp_25_df, filename, overwrite = TRUE)


rm(gpp_25_df, gpp_25_list)
...
#get typical day by which 25% of growth has occurred during years of drought
...
with_progress({
  ...
  gpp_25_drought_list <- future_lapply(id_list_1, function(i) {
    Sys.sleep(0.1)
    p(sprintf("i=%g", i))
    get_25_gpp_drought(i)
  })
})

gpp_25_drought_df <- do.call('rbind', gpp_25_drought_list)
...
gpp_25_drought_df_2 <- do.call('rbind', gpp_25_drought_list_2)
rm(gpp_25_drought_list_2)

gpp_25_drought_df_3 <- rbind(gpp_25_drought_df_2, gpp_25_drought_df)
#head(gpp_25_drought_df_3)

gpp_25_drought <- rasterFromXYZ(gpp_25_drought_df_3)
crs(gpp_25_drought) <- "+proj=longlat +datum=WGS84"
#plot(gpp_25_df)
filename <-
  paste0('./../../Data/CDD/day_of_25/day_25_drought',
         Ecoregion,
         '.tif')
